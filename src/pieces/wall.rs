    ...
    pub fn render(pos: Vector, board: &Board) -> char {
        ...
        if pos.y != 0 {
            if let Some(piece) = &board[pos-Vector::new(0,1)] {
                if piece.wall_connectable() { up = true; count += 1 }
            }
        }
        if pos.y != board.y-1 {
            if let Some(piece) = &board[pos+Vector::new(0,1)] {
                if piece.wall_connectable() { down = true; count += 1 }
            }
        }
        if pos.x != 0 {
            if let Some(piece) = &board[pos-Vector::new(1,0)] {
                if piece.wall_connectable() { left = true; count += 1 }
            }
        }
        if pos.x != board.x-1 {
            if let Some(piece) = &board[pos+Vector::new(1,0)] {
                if piece.wall_connectable() { right = true; count += 1 }
            }
        }
        match count {
            4 => QUAD,
            3 => {
                if !up { return NOT_UP }
                if !left { return NOT_LEFT }
                if !down { return NOT_DOWN }
                NOT_RIGHT
            }
            2 => {
                if up {
